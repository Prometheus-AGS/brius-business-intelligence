# Document Upload MIME Types

## 🎯 **What's Included**

### **1. Table Schema**
- ✅ `id` (UUID, primary key with `gen_random_uuid()`)
- ✅ `mime` (TEXT, unique, non-null)
- ✅ `name` (TEXT, non-null, human-readable)
- ✅ `metadata` (JSONB, with useful info)
...
### **2. All 56 MIME Types** Organized by Category:
- **Word Processing** (8 types): DOC, DOCX, ODT, RTF, etc.
- **PDF** (1 type): application/pdf
- **Presentations** (4 types): PPT, PPTX, PPTM, POT
- **Spreadsheets** (5 types): XLS, XLSX, CSV, TSV, FODS
- **Email** (3 types): EML, MSG, P7S
- **Web** (3 types): HTML, HTM, XML
- **Text** (4 types): TXT, MD, RST, ORG
- **Images** (5 types): JPG, PNG, BMP, TIFF, HEIC
- **eBooks** (1 type): EPUB
- **Database** (1 type): DBF
- **Other formats** (21 types): Apple, StarOffice, legacy, etc.

### **3. Rich Metadata** for Each Type:
```json
{
  "extension": ".pdf",
  "category": "document",
...
1. **`get_mime_type_by_extension('.pdf')`** - Lookup by file extension
2. **`get_mime_types_by_category('document')`** - Filter by category
3. **`supports_ocr('application/pdf')`** - Check OCR capability
4. **`supports_tables('text/csv')`** - Check table extraction support

### **5. Materialized View** for Fast Lookups
- `mime_types_summary` - Pre-aggregated by category
- Refresh with `SELECT refresh_mime_types_summary();`

### **6. Auto-Update Trigger**
- `updated_at` automatically sets on any UPDATE

### **7. Verification & Statistics**
The script automatically shows:
- ✅ Total count (should be 56)
- 📊 Breakdown by category
- 🔍 OCR-enabled types
- 📋 Table extraction-enabled types
...
const isValid = await db.execute(sql`
  SELECT EXISTS(
    SELECT 1 FROM mime_types WHERE mime = ${file.mimetype}
  ) as is_valid
`);

// Get processing strategy
const strategy = await db.execute(sql`
  SELECT 
    CASE 
      WHEN (metadata->>'supports_ocr')::boolean = true 
        THEN 'hi_res'
      ELSE 'fast'
    END as strategy
  FROM mime_types
  WHERE mime = ${file.mimetype}
`);

// Get all supported document formats for UI
const formats = await db.execute(sql`
  SELECT * FROM get_mime_types_by_category('document')
`);
```
...
Just execute this script against your Postgres 17 database:

```bash
psql -U postgres -d brius -f mime_types.sql
```

Or in Drizzle migrations:
```bash
# Save as migrations/0004_mime_types.sql
npm run db:push
```

Let me know if you want me to adjust any of the MIME types, metadata fields, or helper functions! 🎉