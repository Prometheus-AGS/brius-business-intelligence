...
-- Migration: 012_create_bi_context_tables
-- Created: 2025-10-23

-- Enable pgvector extension if not already enabled
CREATE EXTENSION IF NOT EXISTS vector;

-- Create enums for BI context management
...
CREATE TABLE IF NOT EXISTS user_contexts (
    ...
    is_anonymous INTEGER NOT NULL DEFAULT 0, -- 0=false, 1=true
...
CREATE INDEX IF NOT EXISTS idx_user_contexts_role_anonymous ON user_contexts(role_id, is_anonymous);
...
CREATE INDEX IF NOT EXISTS idx_analysis_sessions_start_time ON analysis_sessions(start_time DESC);
...
CREATE INDEX IF NOT EXISTS idx_domain_datasets_dataset_id ON domain_datasets(dataset_id);
CREATE INDEX IF NOT EXISTS idx_domain_datasets_domain_type ON domain_datasets(domain_type);
CREATE INDEX IF NOT EXISTS idx_domain_datasets_table_name ON domain_datasets(table_name);
CREATE INDEX IF NOT EXISTS idx_domain_datasets_access_level ON domain_datasets(access_level);
CREATE INDEX IF NOT EXISTS idx_domain_datasets_last_analyzed ON domain_datasets(last_analyzed DESC NULLS LAST);
...
CREATE INDEX IF NOT EXISTS idx_visualization_artifacts_artifact_id ON visualization_artifacts(artifact_id);
CREATE INDEX IF NOT EXISTS idx_visualization_artifacts_session_id ON visualization_artifacts(session_id);
CREATE INDEX IF NOT EXISTS idx_visualization_artifacts_component_name ON visualization_artifacts(component_name);
CREATE INDEX IF NOT EXISTS idx_visualization_artifacts_generation_time ON visualization_artifacts(generation_time DESC);
...
CREATE INDEX IF NOT EXISTS idx_agent_patterns_pattern_id ON agent_architecture_patterns(pattern_id);
CREATE INDEX IF NOT EXISTS idx_agent_patterns_pattern_type ON agent_architecture_patterns(pattern_type);
CREATE INDEX IF NOT EXISTS idx_agent_patterns_usage_count ON agent_architecture_patterns(usage_count DESC);
CREATE INDEX IF NOT EXISTS idx_agent_patterns_success_rate ON agent_architecture_patterns(success_rate DESC);
...
CREATE TABLE IF NOT EXISTS context_states (
    ...
    is_corrupted INTEGER NOT NULL DEFAULT 0, -- 0=false, 1=true
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
);

-- Create indexes for context_states
CREATE INDEX IF NOT EXISTS idx_context_states_state_id ON context_states(state_id);
CREATE INDEX IF NOT EXISTS idx_context_states_session_id ON context_states(session_id);
CREATE INDEX IF NOT EXISTS idx_context_states_last_update ON context_states(last_update DESC);
CREATE INDEX IF NOT EXISTS idx_context_states_corrupted ON context_states(is_corrupted)
WHERE is_corrupted = 1;

-- ============================================================================
-- Enhanced Memory Tables (extending existing memory system)
-- ============================================================================

-- Update existing user_memories table to support context scoping
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_memories' AND column_name = 'content_type') THEN
        ALTER TABLE user_memories ADD COLUMN content_type TEXT DEFAULT 'conversation';
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_memories' AND column_name = 'scope') THEN
        ALTER TABLE user_memories ADD COLUMN scope TEXT DEFAULT 'user';
        CREATE INDEX IF NOT EXISTS idx_user_memories_scope ON user_memories(scope);
    END IF;
END
$$;

-- Update existing global_memories table to support context scoping
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'global_memories' AND column_name = 'content_type') THEN
        ALTER TABLE global_memories ADD COLUMN content_type TEXT DEFAULT 'knowledge';
        CREATE INDEX IF NOT EXISTS idx_global_memories_content_type ON global_memories(content_type);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'global_memories' AND column_name = 'scope') THEN
        ALTER TABLE global_memories ADD COLUMN scope TEXT DEFAULT 'global';
        CREATE INDEX IF NOT EXISTS idx_global_memories_scope ON global_memories(scope);
    END IF;
END
$$;
...
DECLARE
    ...
    max_entries INTEGER := 100; -- From environment configuration
BEGIN
...
ALTER TABLE domain_datasets ENABLE ROW LEVEL SECURITY;
ALTER TABLE visualization_artifacts ENABLE ROW LEVEL SECURITY;
ALTER TABLE context_states ENABLE ROW LEVEL SECURITY;

-- Policies for user_contexts (users can access only their own contexts)
CREATE POLICY user_contexts_self_access ON user_contexts
    FOR ALL USING (
        user_id::text = auth.uid()::text OR
        (is_anonymous = 1 AND session_id::text = current_setting('app.session_id', true))
    );

-- Service role can access all contexts
CREATE POLICY user_contexts_service_access ON user_contexts
    FOR ALL USING (auth.role() = 'service_role');

-- Policies for analysis_sessions
CREATE POLICY analysis_sessions_user_access ON analysis_sessions
    FOR ALL USING (user_id::text = auth.uid()::text);

CREATE POLICY analysis_sessions_service_access ON analysis_sessions
    FOR ALL USING (auth.role() = 'service_role');

-- Policies for visualization_artifacts (via session ownership)
CREATE POLICY visualization_artifacts_user_access ON visualization_artifacts
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM analysis_sessions
            WHERE analysis_sessions.session_id = visualization_artifacts.session_id
            AND analysis_sessions.user_id::text = auth.uid()::text
        )
    );

CREATE POLICY visualization_artifacts_service_access ON visualization_artifacts
    FOR ALL USING (auth.role() = 'service_role');

-- Policies for context_states (via session ownership)
CREATE POLICY context_states_user_access ON context_states
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM analysis_sessions
            WHERE analysis_sessions.session_id = context_states.session_id
            AND analysis_sessions.user_id::text = auth.uid()::text
        )
    );

CREATE POLICY context_states_service_access ON context_states
    FOR ALL USING (auth.role() = 'service_role');

-- Domain datasets accessible based on user permissions
CREATE POLICY domain_datasets_access ON domain_datasets
    FOR SELECT USING (
        ...
        (
            ...
            EXISTS (
                ...
                AND (uc.permissions->(domain_type::text))->>'read' = 'true'
            )
        )
    );
...
CREATE INDEX IF NOT EXISTS idx_user_contexts_active
ON user_contexts(user_id, session_id, last_activity)
WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_analysis_sessions_active
ON analysis_sessions(user_id, last_query_time)
WHERE status = 'active';

-- GIN indexes for JSONB columns that will be queried
CREATE INDEX IF NOT EXISTS idx_user_contexts_permissions_gin
ON user_contexts USING GIN (permissions);

CREATE INDEX IF NOT EXISTS idx_analysis_sessions_domain_access_gin
ON analysis_sessions USING GIN (domain_access);

CREATE INDEX IF NOT EXISTS idx_domain_datasets_schema_gin
...
COMMENT ON TABLE user_contexts IS 'User context management for authenticated and anonymous BI sessions';
COMMENT ON COLUMN user_contexts.is_anonymous IS 'Flag for anonymous users using SUPABASE_ANON_KEY (0=authenticated, 1=anonymous)';
...
DO $$
BEGIN
    PERFORM pg_sleep(1); -- Brief pause for table creation

    -- Check all expected tables exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_contexts') THEN
        RAISE EXCEPTION 'Failed to create user_contexts table';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'analysis_sessions') THEN
        RAISE EXCEPTION 'Failed to create analysis_sessions table';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'domain_datasets') THEN
        RAISE EXCEPTION 'Failed to create domain_datasets table';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'visualization_artifacts') THEN
        RAISE EXCEPTION 'Failed to create visualization_artifacts table';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'agent_architecture_patterns') THEN
        RAISE EXCEPTION 'Failed to create agent_architecture_patterns table';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'context_states') THEN
        RAISE EXCEPTION 'Failed to create context_states table';
    END IF;
    ...
    RAISE NOTICE 'BI Context Enhancement migration completed successfully';
END
$$;